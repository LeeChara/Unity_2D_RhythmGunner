...
public class JudgeSystem : MonoBehaviour
{
    ...
    public bool Judge(NoteType noteType, bool isKeyDown = false)
    {
        ...
        if (minTickDiff <= perfectTick) // perfect 판정 범위 내에 있는 경우
        {
            ...
            GameManager.Instance.playerController.PlaySE(noteType.ToString());
        }
    ...
    private void OnPerfect(float tick)
    {
        ...
        GameManager.Instance.uiManager.OnPerfect();
        GameManager.Instance.enemyManager.DestroyEnemy(tick); // 노트와 동일한 tick에 소환된 적 제거
        Vector3 position = new Vector3(0, judgeEffectDistance, 0);
    ...
    private void OnGood(float tick)
    {
        ...
        GameManager.Instance.uiManager.OnGood();
        GameManager.Instance.enemyManager.DestroyEnemy(tick); // 노트와 동일한 tick에 소환된 적 제거
        Vector3 position = new Vector3(0, judgeEffectDistance, 0);
    ...
    public void OnMiss()
    {
        ...
        GameManager.Instance.uiManager.OnMiss();
        Vector3 position = new Vector3(0, judgeEffectDistance, 0);
        ...
        GameManager.Instance.playerController.PlaySE("Miss");
    }
}
